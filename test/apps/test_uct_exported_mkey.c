                       ...
                       const cmd_args_t *cmd_args)
{
    uct_md_attr_v2_t md_attr = {
        .field_mask = UCT_MD_ATTR_FIELD_EXPORTED_MKEY_PACKED_SIZE
    };
    uct_md_mem_reg_params_t params;
    uct_md_mkey_pack_params_t mkey_pack_params;
    uint8_t *exported_mkey_buf;
    uint64_t exported_mkey;
    ucs_status_t status;
    uct_mem_h memh;
    void *ptr;
    int ret;
    ...
    params.flags      = UCT_MD_MEM_FLAG_EXPORTED | UCT_MD_MEM_ACCESS_ALL;
    ...
    exported_mkey_buf             =
            (uint8_t*)alloca(md_attr.exported_mkey_packed_size);
    mkey_pack_params.field_mask = UCT_MD_MKEY_PACK_FIELD_FLAGS;
    mkey_pack_params.flags      = UCT_MD_MKEY_PACK_FLAG_EXPORTED;
    status                      = uct_md_mkey_pack_v2(md, memh,
                                                      &mkey_pack_params,
                                                      exported_mkey_buf);
    CHKERR_JUMP(UCS_OK != status, "uct_md_mkey_pack", error_ret);

    exported_mkey = *((uint64_t*)&exported_mkey_buf[0]);
    printf("exported ptr %p len %zu exported_mkey 0x%zx\n", ptr,
           cmd_args->size, exported_mkey);
                       ...
                       const cmd_args_t *cmd_args)
{
    ...
    status = uct_md_mem_attach(md, &cmd_args->mkey, &attach_params, &memh);
    CHKERR_ACTION(UCS_OK != status, "uct_md_mem_attach", return status);

    printf("imported exported mkey: memh=%p\n", memh);
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, "d:i:s:a:")) != -1) {
        switch (c) {
        case 'd':
            args.md_name = optarg;
            break;
        ...
        case 'a':
            ...
            break;
        default:
            printf("Usage: %s [-d <md_name>] [-i <mkey>] [ -s size ] [ -a align ]\n",
                   argv[0]);
            return -1;
        }
    }
