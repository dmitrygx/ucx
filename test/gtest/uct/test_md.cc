...
bool test_md::check_caps(uint64_t flags)
{
    ...
    ucs_status_t status = uct_md_query(uct_md, &md_attr);
    ASSERT_UCS_OK(status);
...
UCS_TEST_SKIP_COND_P(test_md, rkey_ptr, check_caps(UCT_MD_FLAG_ALLOC |
...
UCS_TEST_SKIP_COND_P(test_md, alloc, check_caps(UCT_MD_FLAG_ALLOC)) {
...
UCS_TEST_SKIP_COND_P(test_md, reg, check_caps(UCT_MD_FLAG_REG)) {
    size_t size;
    uct_md_attr_t md_attr;
    ucs_status_t status;
    void *address;
...
UCS_TEST_SKIP_COND_P(test_md, reg_perf, check_caps(UCT_MD_FLAG_REG)) {
...
UCS_TEST_SKIP_COND_P(test_md, reg_advise, check_caps(UCT_MD_FLAG_REG |
                                                     UCT_MD_FLAG_ADVISE)) {
    size_t size;
    ucs_status_t status;
    void *address;
...
UCS_TEST_SKIP_COND_P(test_md, alloc_advise, check_caps(UCT_MD_FLAG_ALLOC |
                                                       UCT_MD_FLAG_ADVISE)) {
    size_t size, orig_size;
    ucs_status_t status;
    void *address;
