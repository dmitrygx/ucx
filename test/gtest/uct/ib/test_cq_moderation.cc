...
protected:
    ...
    void init() {
        ...
        m_sender = uct_test::create_entity(0, NULL, NULL, NULL, NULL, NULL,
                                           async_event_handler,
                                           &m_send_async_event_ctx);
        m_entities.push_back(m_sender);

        m_receiver = uct_test::create_entity(0, NULL, NULL, NULL, NULL, NULL,
                                             async_event_handler,
                                             &m_recv_async_event_ctx);
        m_entities.push_back(m_receiver);

        check_skip_test();

        m_send_async_event_ctx.check_event(*m_sender, 0);
        m_recv_async_event_ctx.check_event(*m_receiver, 0);
    }

    void cleanup() {
        m_send_async_event_ctx.reset();
        m_recv_async_event_ctx.reset();
    ...
    static ucs_status_t async_event_handler(void *arg, unsigned flags) {
        ...
        EXPECT_TRUE((ctx == &m_send_async_event_ctx) ||
                    (ctx == &m_recv_async_event_ctx));
    ...
    void iface_arm(entity &test_e, async_event_ctx &ctx) {
        ...
        } while (ctx.check_event(test_e, 0));
    }
    ...
    static uct_test::async_event_ctx m_send_async_event_ctx;
    static uct_test::async_event_ctx m_recv_async_event_ctx;
};

uct_test::async_event_ctx test_uct_cq_moderation::m_send_async_event_ctx;
uct_test::async_event_ctx test_uct_cq_moderation::m_recv_async_event_ctx;
