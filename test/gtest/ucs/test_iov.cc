...
protected:
    ...
    void expect_zero_changes(size_t res_cnt, size_t res_length,
        ...
        EXPECT_EQ(0lu, iov_iter->iov_offset);
    ...
    void test_iov_type_pair(T1 *iov1, size_t iov1_cnt,
        ...
        while (iov_iter.iov_offset < iov2_cnt) {
            cnt    = iov1_cnt;
            length = iov_converter(iov1, &cnt,
                                   iov2, iov2_cnt,
                                   max_length, &iov_iter);
            EXPECT_TRUE((iov_iter.iov_offset == iov2_cnt) ||
...
UCS_TEST_F(test_ucs_iov, total_length) {
    ...
    EXPECT_EQ(total_length, ucs_iov_total_length(iov, iov_cnt));
