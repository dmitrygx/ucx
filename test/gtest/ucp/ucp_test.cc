...
void ucp_test::entity_disconnect(entity &e, size_t sender_idx)
{
    void *close_req = e.disconnect_nb(0, sender_idx, UCP_EP_CLOSE_MODE_FORCE);
...
void
ucp_test::send_tag_rndv_message_wait(entity &sender, entity &receiver,
                                     ...
                                     size_t sender_idx)
{
    void *sreq = send(sender, buf, length, SEND_RECV_TAG, send_cb, NULL,
                      sender_idx);
...
void ucp_test::complete_err_handling_status_verify(ucs_status_t status)
{
...
void ucp_test::scomplete_cb(void *req, ucs_status_t status)
{
    if ((status == UCS_OK) ||
...
void ucp_test::scomplete_cbx(void *req, ucs_status_t status, void *user_data)
{
    ASSERT_EQ(NULL, user_data);
    scomplete_cb(req, status);
}

void ucp_test::scomplete_always_ok_cbx(void *req, ucs_status_t status,
                                       void *user_data)
{
    ASSERT_EQ(NULL, user_data);
...
void ucp_test::scomplete_reset_data_cbx(void *req, ucs_status_t status,
                                        void *user_data)
{
...
void ucp_test::scomplete_err_handling_cb(void *req, ucs_status_t status)
{
    complete_err_handling_status_verify(status);
}

void ucp_test::rtag_complete_cb(void *req, ucs_status_t status,
                                ucp_tag_recv_info_t *info)
{
...
void ucp_test::rtag_complete_cbx(void *req, ucs_status_t status,
...
void ucp_test::rtag_complete_always_ok_cbx(void *req, ucs_status_t status,
                                           const ucp_tag_recv_info_t *info,
                                           void *user_data)
{
    ASSERT_EQ(NULL, user_data);
...
void
ucp_test::rtag_complete_check_data_cbx(void *req, ucs_status_t status,
                                       ...
                                       void *user_data)
{
    ...
    std::cout << "completed with " << status << " - "  << i++ << std::endl;
...
void ucp_test::rtag_complete_err_handling_cb(void *req, ucs_status_t status,
                                             ucp_tag_recv_info_t *info)
{
    complete_err_handling_status_verify(status);
}

void ucp_test::rstream_complete_cb(void *req, ucs_status_t status,
                                   size_t length)
{
    EXPECT_TRUE((status == UCS_OK) || (status == UCS_ERR_CANCELED));
}

void ucp_test::rstream_complete_cbx(void *req, ucs_status_t status,
...
bool ucp_test::check_send_status(ucs_status_t send_status, entity &receiver,
                                 void* recv_req,
...
void* ucp_test::send(entity& from, const void *contig_buffer, size_t length,
                     send_recv_type_t send_type, ucp_send_nbx_callback_t cb,
                     void *user_data, size_t ep_index)
{
    ...
    if (send_type == SEND_RECV_TAG) {
        ...
        return ucp_am_send_nbx(ep, 0, NULL, 0, contig_buffer, length,
                               &params);
    }
...
void* ucp_test::recv(entity& to, void *contig_buffer, size_t length,
...
ucs_status_t ucp_test::rx_am_msg_cb(void *arg, const void *header,
...
void ucp_test::set_am_data_handler(entity &e, uint16_t am_id,
...
void ucp_test::send_recv(entity& from, entity& to,
                         send_recv_type_t send_recv_type, bool wakeup,
                         ucp_test_base::entity::listen_cb_type_t cb_type,
                         size_t ep_index)
{
    ...
    void *send_req = send(from, &send_data, sizeof(send_data), send_recv_type,
                          scomplete_cbx, NULL, ep_index);

    void *recv_req = NULL; // to suppress compiler warning
    if (send_recv_type == SEND_RECV_TAG) {
        recv_req = recv(to, &recv_data, sizeof(recv_data), rtag_complete_cbx,
                        NULL);
