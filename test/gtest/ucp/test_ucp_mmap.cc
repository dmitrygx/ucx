...
UCS_TEST_P(test_ucp_mmap, rereg)
{
    ...
    for (int i = 0; i < (100 / ucs::test_time_multiplier()); ++i) {
        ...
        status = ucp_mem_map(sender().ucph(), &params, &memh); // 1 (region) + 1 (invalidate)
        ...
        if (get_variant_value() != VARIANT_NO_RCACHE) {
            ...
            for (int i = 0; i < num_iters; ++i) {
                ...
                status = ucp_mem_unmap(sender().ucph(), test_memh); // 2
                ASSERT_UCS_OK(status);
            }
        }

        status = ucp_mem_unmap(sender().ucph(), memh); // 1 - 1 = 0
        ASSERT_UCS_OK(status);
    }
}
