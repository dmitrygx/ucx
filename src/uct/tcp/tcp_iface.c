...
static ucs_config_field_t uct_tcp_iface_config_table[] = {
  ...
  {"BUF_MPOOL_ELEMS_PER_CHUNK", "8",
   "Number of elements per chunk allocated from memory pool for TX/RX operations.",
   ucs_offsetof(uct_tcp_iface_config_t, buf_mpool_elems_per_chunk), UCS_CONFIG_TYPE_MEMUNITS},

  {"BUF_MPOOL_MAX_ELEMS", "64",
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    self->config.buf_mpool.elems_per_chunk = config->buf_mpool_elems_per_chunk;
    ...
    status = ucs_mpool_init(&self->tx_buf_mp, 0, self->config.buf_size,
                            ...
                            self->config.buf_mpool.elems_per_chunk,
                            self->config.buf_mpool.max_elems,
                            &buf_mp_ops, "uct_tcp_iface_tx_buf_mp");
    if (status != UCS_OK) {
        return status;
    }

    status = ucs_mpool_init(&self->rx_buf_mp, 0, self->config.buf_size * 2,
                            0, UCS_SYS_CACHE_LINE_SIZE,
                            self->config.buf_mpool.elems_per_chunk,
                            self->config.buf_mpool.max_elems,
                            &buf_mp_ops, "uct_tcp_iface_tx_buf_mp");
...
err_cleanup_rx_mpool:
    ucs_mpool_cleanup(&self->rx_buf_mp, 1);
err_cleanup_tx_mpool:
    ucs_mpool_cleanup(&self->tx_buf_mp, 1);
    return status;
}
