...
{
    if (ucs_likely(iface->ext_ep_count != 0)) {
        /* optimization: get time of the last async wakeup */
        return iface->super.super.worker->async->last_wakeup;
    } else {
        /* async timer is not used, should get current time value */
        return ucs_get_time();
    }
}
