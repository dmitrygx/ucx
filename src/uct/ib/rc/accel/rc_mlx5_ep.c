...
#endif
        return uct_rc_mlx5_ep_put_short_inline(tl_ep, buffer, length,
                                               remote_addr, rkey);
#if HAVE_IBV_DM
    }
                        ...
                        const void *payload, unsigned length)
{
    ...
    if (ucs_likely(sizeof(uct_rc_mlx5_am_short_hdr_t) +
                   length <= UCT_IB_MLX5_AM_MAX_SHORT(0) || !dm.dm)) {
#endif
    ...
    status = uct_rc_mlx5_ep_short_dm(ep, &cache, sizeof(cache.am_hdr),
                                     payload, length, MLX5_OPCODE_SEND,
                                     MLX5_WQE_CTRL_SOLICITED |
                                            ...
                                            const void *data, size_t length)
{
    ...
    status = uct_rc_mlx5_ep_short_dm(ep, &cache, sizeof(cache.tm_hdr),
