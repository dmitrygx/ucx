...
enum {
    ...
    UCT_IB_MEM_FLAG_NO_RCACHE        = UCS_BIT(5), /**< Memory handle wasn't stored
                                                        in RCACHE */
    UCT_IB_MEM_FLAG_REGISTERED       = UCS_BIT(6), /**< Memory handle was created with
                                                        normal registration */
    UCT_IB_MEM_FLAG_EXPORTED         = UCS_BIT(7), /**< Memory handle was created with
                                                        the ability to share an access
                                                        to a memory buffer with a peer
                                                        for doing operations */
    UCT_IB_MEM_FLAG_IMPORTED         = UCS_BIT(8)  /**< Memory handle was created to
                                                        import memory registration of
...
typedef ucs_status_t (*uct_ib_md_reg_exported_key_func_t)(
        uct_ib_md_t *ib_md, uint64_t flags, void *address, size_t length,
...
static UCS_F_ALWAYS_INLINE uint32_t uct_ib_md_lkey(uint64_t shared_mkey)
{
    return (uint32_t)shared_mkey;
}


static UCS_F_ALWAYS_INLINE uint32_t uct_ib_md_vhca_id(uint64_t shared_mkey)
{
