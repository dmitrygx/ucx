               ...
               const uct_md_mem_reg_params_t *params, uct_mem_h *memh_p)
{
    uint64_t flags      = UCT_MD_MEM_REG_FIELD_VALUE(params, flags,
                                                     FIELD_FLAGS, 0);
    uct_ib_md_t *md     = ucs_derived_of(uct_md, uct_ib_md_t);
    uint32_t memh_flags = (flags & UCT_MD_MEM_FLAG_EXPORT) ?
                          (UCT_IB_MEM_FLAG_NO_RCACHE |
                           UCT_IB_MEM_FLAG_EXPORTED) :
                          UCT_IB_MEM_FLAG_REGISTERED;
    ucs_status_t status;
    uct_ib_mem_t *memh;
    ...
    uct_ib_mem_init(memh, memh_flags);

    if (memh->flags & UCT_IB_MEM_FLAG_EXPORTED) {
        status = md->ops->reg_exported_key(md, flags, address, length, memh);
    } else {
                     ...
                     uct_md_mem_attach_params_t *params, uct_mem_h *memh_p)
{
    ...
    if (mkey == NULL) {
        uct_md_log_mem_reg_error(flags,
                                 "shared_mkey_buffer shouldn't be NULL");
        return UCS_ERR_INVALID_PARAM;
    }
    ...
    uct_ib_mem_init(ib_memh,
                    UCT_IB_MEM_FLAG_NO_RCACHE | UCT_IB_MEM_FLAG_IMPORTED);
                 ...
                 void *mkey_buffer)
{
    ...
    if ((memh->flags & (UCT_IB_MEM_ACCESS_REMOTE_ATOMIC |
        ...
        !(memh->flags &
          (UCT_IB_MEM_FLAG_ATOMIC_MR |  UCT_IB_MEM_FLAG_EXPORTED)) &&
        (memh != md->global_odp))
    {
                                             ...
                                             uint16_t rcache_mem_reg_flags)
{
    ...
    uct_ib_mem_init(&region->memh, UCT_IB_MEM_FLAG_REGISTERED);
