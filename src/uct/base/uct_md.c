...
void uct_md_copy_v1_fields(uct_md_attr_t *dst, uct_md_attr_v2_t *src)
{
    ...
    memcpy(&dst->component_name, &src->component_name, sizeof(src->component_name));
}

void uct_md_copy_v2_fields(uct_md_attr_v2_t *dst, uct_md_attr_v2_t *src)
{
    if (dst->field_mask & UCT_MD_ATTR_FIELD_MAX_ALLOC) {
        dst->max_alloc        = src->max_alloc;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_MAX_REG) {
        dst->max_reg          = src->max_reg;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_FLAGS) {
        dst->flags            = src->flags;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_REG_MEM_TYPES) {
        dst->reg_mem_types    = src->reg_mem_types;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_DETECT_MEM_TYPES) {
        dst->detect_mem_types = src->detect_mem_types;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_ALLOC_MEM_TYPES) {
        dst->alloc_mem_types  = src->alloc_mem_types;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_ACCESS_MEM_TYPES) {
        dst->access_mem_types = src->access_mem_types;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_DMABUF_MEM_TYPES) {
        dst->dmabuf_mem_types = src->dmabuf_mem_types;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_REG_COST) {
        dst->reg_cost         = src->reg_cost;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_RKEY_PACKED_SIZE) {
        dst->rkey_packed_size = src->rkey_packed_size;
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_COMPONENT_NAME) {
        memcpy(&dst->local_cpus, &src->local_cpus, sizeof(src->local_cpus));
    }

    if (dst->field_mask & UCT_MD_ATTR_FIELD_LOCAL_CPUS) {
        memcpy(&dst->component_name, &src->component_name, sizeof(src->component_name));
    }
}
...
uct_md_attr_v2_copy(uct_md_attr_v2_t *dst, const uct_md_attr_v2_t *src)
{
    ...
    UCT_MD_ATTR_V2_FIELD_COPY(dst, src, reg_cost,
