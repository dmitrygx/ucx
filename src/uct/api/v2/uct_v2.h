...
typedef struct {
    ...
    /**
     * Global id - unique device identifier in a system (can be MAC address or
     * GUID), '\0' means that an ID is unknown and should not be used
     */
 ...
 * into a memory buffer which then could be deserialized by a peer and used
 * in UCT operations.
 *
 ...
 * @param [in]  params      Operation parameters, see @ref
 *                          uct_md_mkey_pack_params_t.
 * @param [out] buffer      Pointer to a buffer to hold the packed remote key.
