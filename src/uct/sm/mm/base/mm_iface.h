 ...
 * avoidance. */
#define UCT_MM_IFACE_FIFO_MIN_WINDOW            1 /* Minimal window size value */
#define UCT_MM_IFACE_FIFO_MAX_WINDOW           16 /* Default value for maximal window size */
...
typedef struct uct_mm_iface_config {
    ...
    size_t                   fifo_max_wnd;            /* Maximal RX completions to pick
                                                       * during RX poll */
...
typedef struct uct_mm_iface {
    ...
    unsigned                fifo_wnd;            /* FIFO window size */
