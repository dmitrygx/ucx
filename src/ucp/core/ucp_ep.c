...
static void ucp_ep_config_rndv_zcopy_set(ucp_context_t *context, uint64_t cap_flag,
                                         ucp_lane_index_t lane,
                                         const uct_md_attr_t *md_attr,
                                         const uct_iface_attr_t *iface_attr,
                                         double max_bw[UCS_MEMORY_TYPE_LAST],
...
static void ucp_ep_config_rndv_zcopy_commit(ucp_lane_index_t lanes_count,
                                            ucp_rndv_zcopy_t *rndv_zcopy)
{
    if (lanes_count == 0) {
                                ...
                                const ucp_ep_config_key_t *key)
{
    ...
    for (i = 0; (i < config->key.num_lanes) &&
        ...
        if (rsc_index != UCP_NULL_RESOURCE) {
            ...
            /* GET Zcopy */
            ucp_ep_config_rndv_zcopy_set(context, UCT_IFACE_FLAG_GET_ZCOPY, lane,
                                         md_attr, iface_attr, get_zcopy_max_bw,
                                         &config->rndv.get_zcopy,
                                         &get_zcopy_lane_count);

            /* PUT Zcopy */
            ucp_ep_config_rndv_zcopy_set(context, UCT_IFACE_FLAG_PUT_ZCOPY, lane,
                                         md_attr, iface_attr, put_zcopy_max_bw,
    ...
    /* GET Zcopy */
    ucp_ep_config_rndv_zcopy_commit(get_zcopy_lane_count, &config->rndv.get_zcopy);

    /* PUT Zcopy */
    ucp_ep_config_rndv_zcopy_commit(put_zcopy_lane_count, &config->rndv.put_zcopy);
