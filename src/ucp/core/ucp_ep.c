...
void ucp_ep_destroy_base(ucp_ep_h ep)
{
    ...
    ucs_free(ucp_ep_ext_control(ep));
    ucp_ep_proto_reqs_purge(ep, UCS_ERR_CANCELED);
                             ...
                             ucs_status_t status, int recursive)
{
    ...
    if (req->flags & (UCP_REQUEST_FLAG_SEND_AM | UCP_REQUEST_FLAG_SEND_TAG)) {
        ...
        /* it means that purging started from a request responsible for sending RTR,
         * so a request is responsible for copying data from staging buffer and it
...
void ucp_ep_reqs_purge(ucp_ep_h ucp_ep, ucs_status_t status)
{
    ...
    ucp_ep_proto_reqs_purge(ucp_ep, status);

    if (ucp_ep->flags & (UCP_EP_FLAG_ON_MATCH_CTX | UCP_EP_FLAG_CLOSE_REQ_VALID)) {
