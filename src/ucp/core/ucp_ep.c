                                ...
                                const char *message, ucp_ep_h *ep_p)
{
    ...
    ucp_ep_ext_gen(ep)->num_flush_inprog   = 0;
    ucp_ep_ext_gen(ep)->num_discard_inprog = 0;
#endif
...
{
    ucp_ep_cnt_dec(&ep->ref_cnt);
    if (ep->ref_cnt != 0) {
        return;
    }

    ucs_assert(ucp_ep_ext_gen(ep)->num_flush_inprog == 0);
    ucs_assert(ucp_ep_ext_gen(ep)->num_discard_inprog == 0);
...
{
#if UCS_ENABLE_ASSERT
    uint8_t num_inprog = ucp_ep_ext_gen(ep)->num_discard_inprog +
                         ucp_ep_ext_gen(ep)->num_flush_inprog;
#endif
    ucp_lane_index_t lane;
    uct_ep_h uct_ep;
    ...
    for (lane = 0; lane < ucp_ep_num_lanes(ep); ++lane) {
         ...
         * flush operations started for the UCP EP' + 'number of discarded
         * operations started for the UCP EP' + '1').
        ...
        if (ucp_is_uct_ep_failed(uct_ep)) {
            ucs_assert((ep->ref_cnt == num_inprog) ||
                       (ep->ref_cnt == (num_inprog + 1)));
        } else {
            ucs_assert(ep->ref_cnt == (num_inprog + 1));
        }
#endif
