    ...
    for (lane = 0; lane < config->key.num_lanes; ++lane) {
        ...
        if (rsc_index != UCP_NULL_RESOURCE) {
            ...
            if (iface_attr->cap.flags & UCT_IFACE_FLAG_PUT_ZCOPY) {
                rma_config->put.max_zcopy = iface_attr->cap.put.max_zcopy;
                /* TODO: formula */
                if (context->config.ext.zcopy_thresh == UCS_MEMUNITS_AUTO) {
                    zcopy_thresh = 16384; 
                } else {
                ...
                zcopy_thresh = ucs_max(zcopy_thresh,
                                       iface_attr->cap.put.min_zcopy);
                ucp_ep_config_set_memtype_zcopy_thresh(md_attr,
                                                       rma_config->put.zcopy_thresh,
                                                       zcopy_thresh);
            }
            if (iface_attr->cap.flags & UCT_IFACE_FLAG_PUT_BCOPY) {
                rma_config->put.max_bcopy = ucs_min(iface_attr->cap.put.max_bcopy,
                                                    rma_config->put.zcopy_thresh[UCS_MEMORY_TYPE_HOST]);
            }
            if (iface_attr->cap.flags & UCT_IFACE_FLAG_PUT_SHORT) {
                rma_config->put.max_short = (context->num_mem_type_detect_mds ? -1 :
                                             ucs_min(iface_attr->cap.put.max_short,
                                                     rma_config->put.max_bcopy));
            }

            /* GET */
            if (iface_attr->cap.flags & UCT_IFACE_FLAG_GET_ZCOPY) {
                /* TODO: formula */
                rma_config->get.max_zcopy = iface_attr->cap.get.max_zcopy;
                if (context->config.ext.zcopy_thresh == UCS_MEMUNITS_AUTO) {
                    zcopy_thresh = 16384;
                } else {
                    zcopy_thresh = context->config.ext.zcopy_thresh; 
                ...
                zcopy_thresh = ucs_max(zcopy_thresh,
                                       iface_attr->cap.get.min_zcopy);
                ucp_ep_config_set_memtype_zcopy_thresh(md_attr,
                                                       rma_config->get.zcopy_thresh,
                                                       zcopy_thresh);
            }
            if (iface_attr->cap.flags & UCT_IFACE_FLAG_GET_BCOPY) {
                rma_config->get.max_bcopy = ucs_min(iface_attr->cap.get.max_bcopy,
                                                    rma_config->get.zcopy_thresh[UCS_MEMORY_TYPE_HOST]);
            }
            if (iface_attr->cap.flags & UCT_IFACE_FLAG_GET_SHORT) {
                rma_config->get.max_short = (context->num_mem_type_detect_mds ? -1 :
                                             ucs_min(iface_attr->cap.get.max_short,
                                                     rma_config->get.max_bcopy));
            }
        } else {
