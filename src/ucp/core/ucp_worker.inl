...
#define UCP_SEND_REQUEST_GET_BY_ID(_req_p, _worker, _req_id, _action, \
                                   _fmt_str, ...) \
    { \
...
#define UCP_SEND_REQUEST_EXTRACT_BY_ID(_req_p, _worker, _req_id, _action, \
                                       _fmt_str, ...) \
    { \
        ucs_status_t __status = ucp_send_request_extract_by_id(_worker, \
                                                               _req_id, \
                                                               _req_p); \
        if (ucs_unlikely(__status != UCS_OK)) { \
            ucs_trace_data("worker %p: req id 0x%" PRIx64 " doesn't exist" \
