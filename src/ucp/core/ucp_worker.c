...
ucp_worker_iface_test_event_fd_support(const ucp_worker_iface_t *wiface)
{
    return (wiface->attr.cap.event_flags & UCT_IFACE_FLAG_EVENT_FD) &&
           (wiface->attr.cap.event_flags & UCP_WORKER_UCT_ALL_EVENT_CAP_FLAGS);
}
...
static void ucp_worker_iface_disarm(ucp_worker_iface_t *wiface)
{
    ...
    if (wiface->flags & UCP_WORKER_IFACE_FLAG_ON_ARM_LIST) {
        ucs_assert(ucp_worker_iface_test_event_fd_support(wiface));
...
void ucp_worker_iface_activate(ucp_worker_iface_t *wiface, unsigned uct_flags)
{
    ...
    /* Add to user wakeup */
    if (ucp_worker_iface_test_event_fd_support(wiface)) {
...
static ucs_status_t ucp_worker_iface_event_async_cb(void *arg,
                                                    unsigned flags)
{
    ucp_worker_iface_t *wiface = arg;
...
static void ucp_worker_iface_event(int fd, void *arg)
{
    ucp_worker_iface_t *wiface = arg;
    ucs_status_t status;
                                   ...
                                   ucp_worker_iface_t **wiface_p)
{
    ...
    iface_params->async_event_cb    = ucp_worker_iface_event_async_cb;
...
void ucp_worker_iface_cleanup(ucp_worker_iface_t *wiface)
{
    ...
    if ((wiface->event_fd != -1) &&
        ucp_worker_iface_test_event_fd_support(wiface)) {
