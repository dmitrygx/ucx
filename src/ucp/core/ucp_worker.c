...
static void ucp_worker_put_flush_req(ucp_request_t *req)
{
    ...
    ucp_ep_cnt_dec(&ucp_ep_ext_gen(ucp_ep)->num_discard_inprog);
#endif
...
ucp_worker_discard_uct_ep_pending_cb(uct_pending_req_t *self)
{
    ...
    /* Don't call uct_ep_flush() from pending_cb, because it can be
     * uct_ep_flush(CANCEL) */
    uct_worker_progress_register_safe(worker->uct,
                                      ucp_worker_discard_uct_ep_progress,
                                      req, UCS_CALLBACKQ_FLAG_ONESHOT,
                             ...
                             unsigned ep_flush_flags)
{
    ...
    ucp_ep_cnt_inc(&ucp_ep->ref_cnt);
#if UCS_ENABLE_ASSERT
    ucp_ep_cnt_inc(&ucp_ep_ext_gen(ucp_ep)->num_discard_inprog);
#endif
