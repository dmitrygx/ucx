...
static void ucp_rndv_send_atp(ucp_request_t *sreq, ucs_ptr_map_key_t remote_req_id)
{
                        ...
                        int is_get_zcopy)
{
    ...
    *uct_rkey  = (rkey_index != UCP_NULL_RESOURCE) ?
    ...
    return is_get_zcopy ? ep_config->rndv.get_zcopy_lanes[lane_idx] :
           ep_config->rndv.put_zcopy_lanes[lane_idx];
}
                                   ...
                                   uct_rkey_t uct_rkey, unsigned proto)
{
    ...
    if (proto == UCP_REQUEST_SEND_PROTO_RNDV_GET) {
        ...
        scale     = config->rndv.get_zcopy_scale[lane];
    } else {
        ...
        scale     = config->rndv.put_zcopy_scale[lane];
    }
                                             ...
                                             int is_get_zcopy)
{
    ...
    ucp_lane_index_t *lanes    = is_get_zcopy ? ep_config->rndv.get_zcopy_lanes :
                                              ...
                                              const void *rkey_buf)
{
    ...
    ucp_rndv_req_init_zcopy_lane_map(rndv_req, 1);
                                ...
                                size_t length, size_t offset)
{
    ...
    if (rndv_req == NULL) {
        freq->send.rndv.remote_req_id = UCP_REQUEST_ID_INVALID;
    } else {
        freq->send.rndv.remote_req_id =
                                ...
                                uct_completion_callback_t comp_cb)
{
    ...
    for (i = 0; i < UCP_MAX_LANES; i++) {
        freq->send.rndv.rkey_index[i] = rkey_index ? rkey_index[i] :
                                        UCP_NULL_RESOURCE;
    }
                      ...
                      uct_completion_t *self)
{
    ...
    ucs_trace_req("freq:%p: recv_frag_get done. rreq:%p length:%ld"
                  " offset:%"PRIu64, freq, rndv_req, freq->send.length, offset);
                                 ...
                                 size_t base_offset)
{
    ...
    while (offset != size) {
        ...
        ucp_rndv_send_frag_get_mem_type(rndv_req, remote_req_id, length,
                                        ...
                                        rndv_req->send.rndv.lanes_map_all,
                 ...
                 void *arg, void *data, size_t length, unsigned flags)
{
    ...
    if (UCP_DT_IS_CONTIG(sreq->send.datatype) && rndv_rtr_hdr->address) {
        ...
        if ((context->config.ext.rndv_mode != UCP_RNDV_MODE_GET_ZCOPY) &&
            ...
            ucp_rndv_req_init_zcopy_lane_map(sreq, 0);
