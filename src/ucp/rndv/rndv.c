                                            ...
                                            ucs_status_t status)
{
    ...
    if (status == UCS_OK) {
        ucp_rndv_req_send_ats(rndv_req, rreq, rndv_req->send.rndv.remote_req_id,
                              UCS_OK);
    } else {
...
ucp_rndv_zcopy_get_lane(ucp_request_t *rndv_req, uct_rkey_t *uct_rkey,
                        int is_get_zcopy)
{
    ...
    *uct_rkey  = (rkey_index != UCP_NULL_RESOURCE) ?
    ...
    return is_get_zcopy ? ep_config->rndv.get_zcopy.lanes[lane_idx] :
                                   ...
                                   uct_rkey_t uct_rkey, unsigned proto)
{
    ...
    if ((offset == 0) && (remaining > 0) && (req->send.length > ucp_mtu)) {
        ...
        chunk  = ucs_align_up((size_t)(req->send.length / req->send.rndv.lanes_count
                                       * scale),
                              align);
    ...
    ucs_trace_data("req %p: offset %zu remainder %zu RMA-%s to %p len %zu lane %d",
                 ...
                 uct_pending_req_t *self)
{
    ...
    rndv_req->send.lane = ucp_rndv_zcopy_get_lane(rndv_req, &uct_rkey, 1);
    ...
    return ucp_rndv_progress_rma_zcopy_common(rndv_req, rndv_req->send.lane,
                                              uct_rkey,
...
UCS_PROFILE_FUNC_VOID(ucp_rndv_put_completion, (self), uct_completion_t *self)
{
    ...
    if (sreq->send.state.dt.offset == sreq->send.length) {
        ucp_rndv_send_atp(sreq, sreq->send.rndv.remote_req_id);
    }
}
                                             ...
                                             ucs_memory_type_t mem_type)
{
    ...
    ucp_lane_index_t *lanes    = is_get_zcopy ? ep_config->rndv.get_zcopy.lanes :
    ...
    rndv_req->send.rndv.lanes_map_all = lane_map;
    rndv_req->send.rndv.lanes_count   = ucs_popcount(lane_map);
}
                                              ...
                                              const void *rkey_buf)
{
    ...
    ucp_rndv_req_init_zcopy_lane_map(rndv_req, 1, rndv_req->send.mem_type);
                      ...
                      uct_completion_t *self)
{
    ...
    ucs_ptr_map_key_t rreq_remote_id = freq->send.rndv.remote_req_id;
    ...
    if (!is_put_proto) {
        ...
        if (rndv_req->send.length == rndv_req->send.state.dt.offset) {
            ucp_rkey_destroy(rndv_req->send.rndv.rkey);
            ucp_rndv_req_send_ats(rndv_req, req, rndv_req->send.rndv.remote_req_id,
                                  UCS_OK);
        }
    }
                                ...
                                size_t length, size_t offset)
{
    ...
    freq->send.rndv.remote_req_id  = (rndv_req == NULL) ? UCP_REQUEST_ID_INVALID :
                                     ucp_worker_get_request_id(rreq->recv.worker,
                                                               rndv_req,
                                                               ucp_ep_use_indirect_id
                                                               (freq->send.ep));

    ucp_rndv_req_init_zcopy_lane_map(freq, 0, freq->send.mem_type);
                                ...
                                uct_completion_callback_t comp_cb)
{
    ...
    freq->send.rndv.lanes_map_all     = lanes_map;    
    freq->send.rndv.lanes_count       = ucs_popcount(lanes_map);
    freq->send.rndv.rkey              = rkey;
    freq->send.rndv.remote_address    = remote_address;
    freq->send.rndv.remote_req_id     = rreq_id;

    for (i = 0; i < UCP_MAX_LANES; i++) {
        freq->send.rndv.rkey_index[i] = (rkey_index != NULL) ? rkey_index[i] :
                                        UCP_NULL_RESOURCE;
    }
    ...
    if (UCP_DT_IS_CONTIG(rreq->recv.datatype)) {
        if ((rndv_rts_hdr->address != 0) &&
            ucp_rndv_test_zcopy_scheme_support(rndv_rts_hdr->size,
                                               ep_config->rndv.get_zcopy.min,
                                               ep_config->rndv.get_zcopy.max,
                                               ep_config->rndv.get_zcopy.split)) {
                      ...
                      uct_completion_t *self)
{
    ...
    freq->send.rndv.lanes_count          = fsreq->send.rndv.lanes_count;
    freq->send.rndv.rkey                 = fsreq->send.rndv.rkey;
    freq->send.rndv.remote_address       = fsreq->send.rndv.remote_address +
                                                     ...
                                                     ucp_rndv_rtr_hdr_t *rndv_rtr_hdr)
{
    ...
    while (offset != rndv_size) {
        send_buffer = UCS_PTR_BYTE_OFFSET(fsreq->send.buffer, offset);
        length      = ucp_rndv_adjust_zcopy_length(min_zcopy, max_frag_size, 0,
                                                   rndv_size, offset,
                                                   rndv_size - offset);
        ...
        if (UCP_MEM_IS_HOST(sreq->send.mem_type)) {
            ...
            ucp_rndv_send_frag_get_mem_type(fsreq, 0, length, (uint64_t)send_buffer,
                                            fsreq->send.mem_type, NULL, NULL,
                                            UCS_BIT(0), 0,
    ...
    if (UCP_DT_IS_CONTIG(sreq->send.datatype) && rndv_rtr_hdr->address) {
        ...
        if ((context->config.ext.rndv_mode != UCP_RNDV_MODE_GET_ZCOPY) &&
            ucp_rndv_test_zcopy_scheme_support(sreq->send.length,
                                               ep_config->rndv.put_zcopy.min,
                                               ep_config->rndv.put_zcopy.max,
                                               ep_config->rndv.put_zcopy.split)) {
            ...
            ucp_rndv_req_init_zcopy_lane_map(sreq, 0, sreq->send.mem_type);
