...
static size_t ucp_rma_sw_get_req_pack_cb(void *dest, void *arg)
{
    ...
    getreqh->req.req_id = req->send.req_id.local;
                 ...
                 void *arg, void *data, size_t length, unsigned am_flags)
{
    ...
    req->flags         = 0;
    /* no need to initialize local request ID by UCP_REQUEST_ID_INVALID, since
     * local and remote are union */
    req->send.req_id.remote = getreqh->req.req_id;
    req->send.ep            = ep;
                 ...
                 void *arg, void *data, size_t length, unsigned am_flags)
{
    ...
    UCP_SEND_REQUEST_GET_BY_ID(&req, worker, getreph->req_id, return UCS_OK,
