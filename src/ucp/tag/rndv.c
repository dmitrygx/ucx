...
size_t ucp_tag_rndv_rts_pack(void *dest, void *arg)
{
     ...
     * support RNDV PUT Zcopy scheme */
    rndv_rts_hdr->put_zcopy_allowed = ucp_tag_rndv_is_put_zcopy_supported(sreq);
                      ...
                      const ucp_rndv_rts_hdr_t *rndv_rts_hdr)
{
    ...
    if (UCP_DT_IS_CONTIG(rreq->recv.datatype)) {
        ...
        if (rndv_rts_hdr->put_zcopy_allowed) {
            /* put protocol is allowed on a receiver side (i.e. RNDV mode is not
             * GET Zcopy and RNDV pipeline protocol shouldn't be used, and
             * a sender indicated that RNDV PUT Zcopy can be used for this message
             * size - register receive buffer memory for RMA */   
