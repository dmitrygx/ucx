...
/* to prevent destoying UCT EPs that are used by the main EP */
static void ucp_wireup_prevent_destroy_used_eps(ucp_ep_h ep)
{
    ucp_wireup_ep_t *cm_wireup_ep;
    ucp_lane_index_t lane, tmp_lane;

    ucs_assert(ucp_wireup_has_cm_tmp_ep(ep));
    ...
    for (lane = 0; lane < ucp_ep_num_lanes(ep); ++lane) {
        ...
        if (tmp_lane != UCP_NULL_LANE) {
            cm_wireup_ep->tmp_ep->uct_eps[tmp_lane] = NULL;
        }
    }
}
...
void ucp_wireup_destroy_cm_tmp_ep(ucp_ep_h ep, int flush_uct_eps_prior)
{
    ...
    ucp_wireup_prevent_destroy_used_eps(ep);
...
static void ucp_wireup_update_cm_tmp_ep_lanes(ucp_ep_h ep)
{
    ...
    for (lane = 0; lane < ucp_ep_num_lanes(tmp_ep); ++lane) {
        if (lane == ucp_ep_get_cm_lane(tmp_ep)) {
             ...
             * the main EP */
            wireup_ep = ucs_derived_of(tmp_ep->uct_eps[lane],
                                       ucp_wireup_ep_t);
            ucs_assert(wireup_ep->super.ucp_ep == ep);
            wireup_ep->super.ucp_ep = tmp_ep;
                               ...
                               const ucp_unpacked_address_t *remote_address)
{
    ...
    ucp_wireup_update_cm_tmp_ep_lanes(ep);
                                   ...
                                   unsigned *addr_indices)
{
    ...
    int *saved_lanes;
    ucp_lane_index_t saved_lanes_num;
    ucp_ep_config_key_t key, *old_key;
    ...
    if (ep->cfg_index != UCP_WORKER_CFG_INDEX_NULL) {
        ...
        saved_lanes_num = ucs_max(old_key->num_lanes, key.num_lanes);
        saved_lanes     = ucs_alloca(saved_lanes_num * sizeof(*saved_lanes));
        memset(saved_lanes, 0, saved_lanes_num * sizeof(*saved_lanes));
