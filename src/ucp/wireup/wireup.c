...
ucs_status_t ucp_wireup_msg_progress(uct_pending_req_t *self)
{
    ...
    if (packed_len < 0) {
        if (packed_len != UCS_ERR_NO_RESOURCE) {
            ucs_fatal("failed to send wireup: %s",
                         ...
                         const ucp_lane_index_t *lanes2remote)
{
    ...
    for (lane = 0; lane < ucp_ep_num_lanes(ep); ++lane) {
        ...
        if (ucp_wireup_ep_test(ep->uct_eps[lane]) &&
            !(ucp_wireup_ep(ep->uct_eps[lane])->flags &
                              ...
                              const ucp_unpacked_address_t *remote_address)
{
    ...
    if (ep->uct_eps[lane] == NULL) {
        ...
        if (ucp_wireup_ep(uct_ep)->super.uct_ep != NULL) {
            goto out;
        }
    }
...
static void
ucp_wireup_ep_config_reuse_lane_map(ucp_ep_h ep,
                                    ...
                                    ucp_lane_index_t *reuse_lane_map)
{
    ...
    for (; old_lane_idx < old_key->num_lanes; ++old_lane_idx) {
        ...
        reuse_lane_map[old_lane_idx] =
            ucp_wireup_ep_configs_can_reuse_lane(old_key, new_key, old_lane_idx);
    }
}
                                      ...
                                      ucp_lane_index_t new_lane)
{
    ...
    for (old_lane_idx = 0; old_lane_idx < old_key->num_lanes; ++old_lane_idx) {
        if (reuse_lane_map[old_lane_idx] == new_lane) {
            return old_lane_idx;
        }
    }
                                   ...
                                   unsigned *addr_indices)
{
    ...
    if (ep->cfg_index != UCP_WORKER_CFG_INDEX_NULL) {
        ...
        if (ucp_ep_config_key_has_cm_lane(&key)) {
            ucs_assert(ucp_ep_config(ep)->key.wireup_lane != UCP_NULL_LANE);
            ...
            if ((key.wireup_lane != UCP_NULL_LANE) &&
                (UCP_NULL_LANE !=
                 (reuse_lane = ucp_wireup_ep_config_find_reused_lane(
                                   ep, reuse_lane_map, key.wireup_lane)))) {
                if (key.wireup_lane != reuse_lane) {
                    ep->uct_eps[key.wireup_lane] = ep->uct_eps[reuse_lane];
                    ep->uct_eps[reuse_lane]      = NULL;
                }
            } else if (UCP_NULL_LANE !=
                       (reuse_lane = reuse_lane_map[ucp_ep_config(ep)->
                                                    key.wireup_lane])) {
                key.wireup_lane              = reuse_lane;
            } else {
                cm_wireup_ep                 = ucp_ep_get_cm_wireup_ep(ep);
                key.wireup_lane              = ucp_ep_config_key_get_cm_lane(&key);
                reuse_lane                   = ucp_ep_config(ep)->key.wireup_lane;
                /* save UCT EP from the previous WIREUP lane to the CM WIREUP
                 * auxiliary EP */
                cm_wireup_ep->aux_ep         = ep->uct_eps[reuse_lane];
                cm_wireup_ep->aux_rsc_index  = ucp_ep_config(ep)->key.lanes
        ...
        for (lane = 0; lane < ucp_ep_num_lanes(ep); ++lane) {
            ...
            } else if (reuse_lane != lane) {
                ep->uct_eps[reuse_lane] = ep->uct_eps[lane];
                ep->uct_eps[lane]       = NULL;
            }
        }
    }
