...
unsigned ucp_wireup_destroy_cm_tmp_ep_progress(void *arg)
{
...
void ucp_wireup_destroy_cm_tmp_ep(ucp_ep_h ep)
{
    ...
    ucp_wireup_destroy_cm_tmp_ep_progress(cm_wireup_ep->tmp_ep);
...
void ucp_wireup_remote_connected(ucp_ep_h ep)
{
    ...
    /* CM only part when UCP EP still has CM wireup EP and tmp EP */
    cm_wireup_ep = ucp_ep_get_cm_wireup_ep(ep);
    ucs_assert(cm_wireup_ep->tmp_ep != ep);
    ucp_wireup_prevent_destroy_used_eps(ep);

    uct_worker_progress_register_safe(ep->worker->uct,
                                      ucp_wireup_destroy_cm_tmp_ep_progress,
                                      cm_wireup_ep->tmp_ep,
