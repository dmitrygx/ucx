...
/* Note: need_am parameter mustn't be set only to 1, because
 * it should be possible to try to select RMA/AMO transports
 * and then check whether AM emulation is necessary or not */
static UCS_F_NOINLINE ucs_status_t
                               ...
                               int allow_am, int *need_am)
{
    ...
    while (address_count > 0) {
        ...
        if ((status != UCS_OK) ||
            /* - the selected transport isn't scalable enough,
...
static double ucp_wireup_scale_score_func(ucp_context_h context,
                                          const uct_iface_attr_t *iface_attr)
{
    return ((double)iface_attr->max_num_eps /
                                            ...
                                            ucp_err_handling_mode_t err_mode)
{
    ...
    /* - transport selection wasn't OK */
    if ((status != UCS_OK) ||
        /* - the selected TAG transport isn't scalable enough,
         *   and a scalability score of the selected transport
         *   worse than a scalability score of the selected
         *   AM transport */
        (!ucp_wireup_is_scalable_transport(select_info.score.scale) &&
