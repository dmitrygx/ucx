                    ...
                    ucp_wireup_select_context_t *select_ctx)
{
    ...
    return ucp_wireup_add_lane_desc(select_info, addr_entry->md_index,
                       ...
                       ucp_wireup_select_context_t *select_ctx)
{
    ...
    ucp_wireup_init_select_info(0., 0, UCP_NULL_RESOURCE,
                                UCP_NULL_RESOURCE, 0, &select_info);
                                  ...
                                  ucp_lane_index_t num_lanes)
{
    ...
    for (lane = 0; lane < num_lanes; ++lane) {
        if (lane_descs[lane].lane_types & UCS_BIT(UCP_LANE_TYPE_CM)) {
            continue;
        }
                           ...
                           unsigned *addr_indices, ucp_ep_config_key_t *key)
{
    ...
    if (!ucp_ep_init_flags_has_cm(select_params->ep_init_flags) ||
        ucp_cm_tl_bitmap_use_same_device(context,
